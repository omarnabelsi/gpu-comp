...
export function ContactSubmissionsTable({ items }) {
  return (<div className={`${bem("components-dashboard-contact-submissions-table__c1")} x-scrollbar`} style={{ borderRadius: "18px 3px 18px 3px" }}>
        <table className={`${bem("components-dashboard-contact-submissions-table__c2")} min-w-[760px]`}>
          ...
          <tbody>
            {items.map((item) => (<tr key={item.id} className={bem("components-dashboard-contact-submissions-table__c9")}>
                ...
                <td className={`${bem("components-dashboard-contact-submissions-table__c11")} break-all`}>{item.email}</td>
                <td className={`${bem("components-dashboard-contact-submissions-table__c12")} break-words`}>{item.message}</td>
